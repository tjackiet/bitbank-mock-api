...
- **Phase 0（着手初日）に骨子を作る。** 列は「項目 / モックの挙動 / 根拠（公式 doc の節・引用） / 本物との差異 / 推測かどうか / 影響（DCL 仕様・証明への含意）」。メモ 5 節の 7 行に加え、1.2〜1.3 節で見つかった以下をシードとして入れる
  ...
  - `maker_taker` が常に `maker`（指値）なのに手数料は単一の料率（既定 0.12%）で計算している不整合。**括弧内の「要確認」は 2026-09-17 に実測して閉じた**（`GET https://api.bitbank.cc/v1/spot/pairs`、認証不要）。通貨ペアごと・メイカー／テイカー別で異なるのは事実で、さらに **maker は 61 ペアが `-0.0002`（リベート）**、**`btc_jpy` だけ taker `0.001` / maker `0`** だった。指値ではモックが引く側、実 API は受け取る側で**符号が逆になる**。数値と留保（キャンペーンで変動する）は `docs/fidelity.md` の「手数料」行に記録した。**挙動は変えていない**（下の決定 3 のまま）
...
| 5 | 起動時の孤児 `.tmp` 掃除（任意） | 不要 | `loadOrInitDefault()` | 未着手（PR 4 が入ったので着手できる） |
...
| 保持者の判定 | `<状態ファイル>.lock` に pid を 1 行。`process.kill(pid, 0)` で生死を見る。**`EPERM` は生きている側に倒す**（居るが自分に権限が無い、なので奪う根拠が無い） |
...
`tests/store/lock.test.ts`（8 件）がロック自体を、`tests/index.test.ts`（1 件）が**配線**を見る。
...
ロック側の 4 つの退行も隔離コピーで確かめ、いずれも意図した 1 件だけが落ちた。
