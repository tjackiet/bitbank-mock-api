...
| 絞り込みパラメータの不正値 | `active_orders` / `trade_history` の `count` / `from_id` / `end_id` / `since` / `end` は、**空文字と空白だけの値を「未指定」や `0` として扱わず**、パラメータごとの error code で断る（**HTTP 200**。上の「封筒に包まれない応答」行）。`count` `40006` / `end` `40007` / `end_id` `40008` / `from_id` `40009` / `since` `40022`。数値として読めない値（`?count=abc`）と、同名クエリが 2 本来て配列になった値も同じコード。指定が無いときの挙動は従来どおり | **実 API を実測**（2026-09-16、`btc_jpy`、認証済みの口座）: `active_orders?count=` → `40006`、`active_orders?end=` → `40007`、`trade_history?since=` → `40022`。コードの定義は errors.md（`0badd680`）の `"Invalid count."` / `"Invalid end param."` / `"Invalid end_id."` / `"Invalid from_id."` / `"Invalid trading start time."` | 差異なし（この 3 つは実測で一致）。**`end_id` / `from_id` と、実測していないエンドポイント側の組み合わせは errors.md のコード名からの推測**。**空白だけの値（`?end=%20`）を空文字と同じ扱いにするのも推測**（`Number()` から見て空文字と区別が付かないため揃えた。実 API 側は未実測）。複数のパラメータが同時に不正なときに実 API がどれを返すかも未実測 | 一部はい（上記） | v0.1.0 からの**改訂**。旧版は `z.coerce.number()` が `""` を `0` にするため、**`?end=` が `success: 1` のまま常に空配列**を返していた（`0` 以下だけを残す絞り込みになる）。`?count=` は汎用の `20003` だった。Nyx が絞り込みを使うなら、値が空・空白になり得る変数をクエリへ入れないこと。複数不正時のコード選択に依存しないこと |
...
- ペアは `base_quote` の 2 セグメントだけを受け付ける。余剰セグメント（`btc_jpy_x`）と base == quote（`jpy_jpy`）は `INVALID_PAIR`。加えて各セグメントの文字種を `[a-z0-9]+` に限り、記号・空白・制御文字・大文字を含むペア（`../../admin_jpy` / `btc?a=1_jpy` / `btc#frag_jpy`）も `INVALID_PAIR`。不正ペアの発注は `tick()` より前に拒否し、他注文を動かさない
  ...
  - **実在性の検査は未確定（2026-09-17 に実測して論点を更新）。挙動は変えていない。** かつてここに「本物は未知のペアに `10000` を返す」と書いていたが、**誤りだった**。実 API は `GET /v1/user/spot/order?pair=xxx_yyy&order_id=1` と `GET /v1/user/spot/active_orders?pair=xxx_yyy` の両方に **`40017`（"Invalid asset."）** を返す（`btc_jpy` で対照、認証済みの口座）。`xxx_yyy` は文字種としては正しいので、**実 API はペアをホワイトリストで持っている**ことになる。本モックは同じ入力に `GET order` なら `50009`、`active_orders` なら `success: 1` で空配列を返す。揃えるにはペア一覧を持つ必要があり、「未登録ペアも既定桁で動く」という上の決定と正面からぶつかるので、**ここでは決めていない**。Nyx は、モックが受け付けたペアが本物でも受け付けられるとは限らないことを前提にする
