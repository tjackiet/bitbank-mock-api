...
- **推測**: 一部はい。上限が 30 本であることと `60011` の意味は**公式に明記**（いいえ）。**単位を口座全体にしたこと・`INACTIVE` を除くこと・成行も数えること・他のコードとの優先順は推測**。単位を口座全体にした理由は 2 つで、(1) 公式の文言が "Simultaneous orders" で**ペアに言及していない**、(2) 口座全体の方が制限が強く **fail-closed 側に倒れる**。**ただし Plan A ではペア単位と口座全体を区別できない**——契約範囲が `btc_jpy` の指値だけなので、どちらの数え方でも同じ結果になる（区別が付かないからこそ、どちらを選んだかを `tests/routes/create-order.test.ts` の「別のペアの未約定注文も同じ 1 本として数える」で固定した）。`INACTIVE` を除くことも同じで、**Plan A に `INACTIVE` へ到達する経路が無い**ため区別が付かない
...
- **推測**: はい（`40001` / `40020` / `40021` / `40024` の選択、取消経路の `40013` / `40014`、複数フィールドが同時に落ちたときの優先順、`20003` の流用）。いいえ（`30009` と `40017` の使い分け、絞り込みパラメータの専用コードは実測）
