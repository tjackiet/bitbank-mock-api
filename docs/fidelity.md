...
口座全体で active な注文が既に **30 本**あるとき、**新規発注**を `60011` で断る（31 本目を拒否する）。判定は `store.tick()` より前なので、断ったときに**状態は一切変わらない**（注文も採番も `updatedAt` も動かない）。数え方は [`src/engine/state.ts`](../src/engine/state.ts) の `activeOrders()` をそのまま使い、`STATUS_KIND` が `"active"` に分類する `UNFILLED` と `PARTIALLY_FILLED` だけを数える。**部分約定した注文は数に入り**、`INACTIVE`（`"pending"`）と終端（`FULLY_FILLED` / 取消系 / `REJECTED`）は入らない。上限は**発注時の検査だけ**で、31 本以上を持つ状態ファイルは読み込みも照会も取消も通す
