...
対応表の「推測」の下で**確認先が実 API** のものだけを集めた索引。**ここは索引であって説明ではない**——
なぜ未確認なのか、モックがいまどちらに倒しているかは、リンク先の小節で文脈ごと読むこと。
問いの文は小節の箇条と一字一句同じにしてあり、`tests/docs.test.ts` が両方向（小節にあって
ここに無い・ここにあって小節に無い）を突き合わせる。確認先が「利用側と合意」「モックの設計判断」の
ものは載せない（実 API に聞いても答えが出ない）。

**優先度の基準**: 次の 2 つの軸で決める。

1. **互換を主張する範囲で通るか** — [`docs/plan-a-readiness.md`](plan-a-readiness.md) の
   「この文書が互換を主張する範囲」（`btc_jpy`・指値・互換ルートの応答の形と失敗の見え方）の中で、
   利用側が実際に踏む経路か
2. **外れたときに利用側が書き直すか** — 正しく組み立てた要求でも起こり得て、答えがモックと違ったら
   利用側が分岐・パーサ・見積もりを直すことになるか。小節の「利用側への含意」が既に
   「依存しないこと」と指示しているもの、正しい利用側が送らない入力は、これに当たらない

**両方に当たれば高、片方だけなら中、どちらにも当たらなければ低。** 同じ優先度の中は対応表の順に並べる。

### 優先度 高

- [存在しない単一注文 ID](#存在しない単一注文-id) — 存在しない `order_id` を `GET order` に渡したとき、実 API は `50009` を返しますか
- [存在しない単一注文 ID](#存在しない単一注文-id) — 実在する `order_id` に別のペアの `pair` を添えて `GET order` を呼んだとき、実 API は `50009` を返しますか
- [存在しない一括照会 ID](#存在しない一括照会-id) — `orders_info` の `order_ids` に存在しない ID を混ぜたとき、実 API はエラーにせず、その ID を `orders` から除いて返しますか
- [存在しない一括照会 ID](#存在しない一括照会-id) — `order_ids` がすべて存在しない ID のとき、実 API は `success: 1` と空の `orders` を返しますか
- [存在しない一括照会 ID](#存在しない一括照会-id) — 実 API の `orders_info` は、`orders` を `order_ids` に並べた順で返しますか
- [数量・価格の精度](#数量価格の精度) — `amount` が数量の桁（`amount_digits`）に収まらない発注に、実 API は `40001` を返しますか
- [数量・価格の精度](#数量価格の精度) — `price` が価格の桁（`price_digits`）に収まらない発注に、実 API は `40020` を返しますか
- [残高の桁](#残高の桁) — 実 API の `free_amount` は、同じ応答の `onhand_amount - locked_amount` と表示桁で常に一致しますか
- [残高の桁](#残高の桁) — 実 API の `GET /v1/user/assets` の金額は、表示桁へ常に切り捨てて返りますか
- [平均約定価格の丸め](#平均約定価格の丸め) — 実 API の `average_price` は、約定代金 ÷ 約定量を価格桁に四捨五入した値ですか
- [取消済み・約定済みの取消](#取消済み約定済みの取消) — `cancel_orders` に取消済み・約定済みの注文が混ざったとき、実 API は 1 件も取り消さずにエラーを返しますか
- [エラーコード](#エラーコード) — `amount` が非正または数値として読めない発注に、実 API は `40001` を返しますか
- [エラーコード](#エラーコード) — `price` が非正または数値として読めない指値の発注に、実 API は `40020` を返しますか
- [active_orders の絞り込み](#active_orders-の絞り込み) — 実 API の `active_orders` の `since` / `end` は、ミリ秒ですか、秒ですか
- [active_orders の絞り込み](#active_orders-の絞り込み) — 実 API の `active_orders` の `from_id` / `end_id` / `since` / `end` は、境界の値を含みますか
- [trade_history の絞り込み](#trade_history-の絞り込み) — `count` を省いたとき、実 API の `trade_history` は何件返しますか
- [trade_history の絞り込み](#trade_history-の絞り込み) — `count` が 1000 を超えたとき、実 API は断りますか、1000 件に切り詰めますか
- [部分約定を取り消した注文](#部分約定を取り消した注文) — 実 API で `CANCELED_PARTIALLY_FILLED` の注文を照会すると、`executed_amount` は取消前の約定量のまま返りますか
- [部分約定を取り消した注文](#部分約定を取り消した注文) — 部分約定の後に取り消した注文の約定は、実 API の `trade_history` から引けますか
- [封筒に包まれない応答](#封筒に包まれない応答) — 2026-09-17 に測っていない失敗（発注の残高不足・不正値など）にも、実 API は HTTP 200 を返しますか

### 優先度 中

- [注文状態](#注文状態) — 実 API の Fetch order information（`GET order`）は、`REJECTED` を返すことがありますか
- [注文状態](#注文状態) — 実 API の Create new order（`POST order`）の応答は、`REJECTED` を返すことがありますか
- [注文状態](#注文状態) — 実 API の Cancel order の応答は、`REJECTED` を返すことがありますか
- [成行注文の記録](#成行注文の記録) — 実 API で、成行注文の `order_id` を `GET order` で照会できますか
- [成行注文の記録](#成行注文の記録) — 実 API の成行注文の発注応答は、`status` が `FULLY_FILLED` で返りますか
- [拘束額](#拘束額) — `xrp_jpy` 以外のペアや別の注文サイズでも、実 API の指値買いの `locked_amount` の増分は `price × amount × (1 + taker 料率)` ですか
- [数量・価格の精度](#数量価格の精度) — 実 API の trade の `fee_amount_quote` は、jpy のとき小数 4 桁の固定桁で返りますか
- [ペアの実在性](#ペアの実在性) — 公式一覧に無いペアへの `POST order` に、実 API は `40017` を返しますか
- [ペアの実在性](#ペアの実在性) — 発注停止（`stop_order`）のペアへの新規発注に、実 API は `70017` を返しますか
- [残高の桁](#残高の桁) — 実 API の `GET /v1/user/assets` は、jpy 以外の資産の `amount_precision` を 8 で返しますか
- [残高の桁](#残高の桁) — 実 API は負の金額を表示桁へ丸めるとき、0 方向と -∞ 方向のどちらへ倒しますか
- [assets に出る資産](#assets-に出る資産) — 実 API の `GET /v1/user/assets` は、残高が 0 の資産も含めて取扱資産をすべて返しますか
- [取消済み・約定済みの取消](#取消済み約定済みの取消) — `REJECTED` の注文を取り消したとき、実 API は `50009` を返しますか
- [取消済み・約定済みの取消](#取消済み約定済みの取消) — `cancel_orders` の `order_ids` に同じ id を 2 回入れたとき、実 API は 2 件目を黙って飛ばしますか
- [一括取消の件数上限](#一括取消の件数上限) — `cancel_orders` に 31 件以上の `order_ids` を渡したとき、実 API は `40015` を返しますか
- [一括取消の件数上限](#一括取消の件数上限) — 件数の超過で断るとき、実 API は 1 件も取り消しませんか
- [一括取消の件数上限](#一括取消の件数上限) — 31 件以上のうち 1 件が不正値のとき、実 API は `40015` と `40014` のどちらを返しますか
- [同時未約定注文の上限](#同時未約定注文の上限) — 実 API の同時発注の上限 30 件は、口座全体で数えますか、ペアごとに数えますか
- [同時未約定注文の上限](#同時未約定注文の上限) — active な注文が 30 件あるとき、実 API は成行の新規発注も `60011` で断りますか
- [同時未約定注文の上限](#同時未約定注文の上限) — 同時発注の上限と、桁溢れ（`40001` / `40020`）や発注停止（`70017`）に同時に当たったとき、実 API はどのコードを返しますか
- [エラーコード](#エラーコード) — `side` が `buy` / `sell` のどちらでもない発注に、実 API は `40021` を返しますか
- [エラーコード](#エラーコード) — `type` が `limit` / `market` のどちらでもない発注に、実 API は `40024` を返しますか
- [エラーコード](#エラーコード) — `POST order` で複数のフィールドが同時に不正なとき、実 API はどのフィールドのコードを返しますか
- [エラーコード](#エラーコード) — 本文が JSON の object でない要求（配列など）に、実 API は何番のコードを返しますか
- [エラーコード](#エラーコード) — `cancel_order` / `cancel_orders` で `pair` を省いたとき、実 API は `30009` を返しますか
- [エラーコード](#エラーコード) — `cancel_order` / `cancel_orders` の `pair` が不正な値のとき、実 API は `40017` を返しますか
- [エラーコード](#エラーコード) — `trade_history` の `order` に `asc` / `desc` 以外を送ったとき、実 API は何番のコードを返しますか
- [`expire_at`](#expire_at) — 実 API は、通常の指値注文（`type: "limit"`）の `expire_at` に `null` 以外の値を返すことがありますか
- [注文の `canceled_at`](#注文の-canceled_at) — 取消済みの注文を `GET order` / `orders_info` で照会したとき、実 API は `canceled_at` を返しますか
- [注文の `canceled_at`](#注文の-canceled_at) — 取り消していない注文を照会したとき、実 API は `canceled_at` を（`0` などの値で）返しますか
- [約定の固定フィールド](#約定の固定フィールド) — 実 API の trade の `fee_amount_base` は `"0"` ですか、`"0.00000000"` のような固定桁ですか
- [約定の固定フィールド](#約定の固定フィールド) — 実 API の `trade_history` の各 trade は、`fee_occurred_amount_quote` を含みますか
- [信用取引・逆指値の項目](#信用取引逆指値の項目) — 実 API は、現物の trade に `profit_loss` / `interest` のキーを含めますか
- [maker / taker 表示](#maker--taker-表示) — 実 API で、指値注文の約定が `maker_taker: "taker"` になることはありますか
- [封筒に包まれない応答](#封筒に包まれない応答) — `content-type: application/json` で壊れた JSON 本文を送ったとき、実 API は何を返しますか
...
確認先を付けると、「誰かに聞くべきことが残っている」と読ませてしまう。「いいえ」で始まっても、値の中に
**「はい」（一部が推測）か「留保」を書いている小節**（下の「注文状態」節、「拘束額」節）は条件付きのいいえで、
その部分に確認先を付ける。未確認の事項を足すときは、この語のどちらかを「推測」の値に書くこと。形は `tests/docs.test.ts` が見る。
確認先が実 API の問いは、上の「[実 API への問い](#実-api-への問い)」に優先度つきの索引がある。**問いを足したら索引にも足す**（足し忘れは同じテストが落とす）。
