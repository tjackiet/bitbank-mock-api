...
| 取消済み・約定済みの取消 | 取消済み（`CANCELED_*`）は `50026`、約定済み（`FULLY_FILLED`）は `50027`。`REJECTED` は `50009`。`cancel_orders` はリクエスト順で終端が混ざるとエラーを返し、1 件も取消しない。**`order_ids` に同じ id を 2 回以上入れると、その注文は 1 回だけ取り消され応答の `orders` にも 1 件しか載る**（2 件目以降は直前の取消で終端になった注文に当たるため。要求した件数より `orders` が短くなる唯一の経路） | error codes | 公式のバッチ混在時の挙動は未確認。`REJECTED` への取消コードも未明記 | はい（REJECTED とバッチ fail-closed、重複 id の扱い） | 終端状態の識別を保つ。**取消が一部だけ成立する意味での部分成功は起きない。** ただし重複 id を送ると `orders` の件数が `order_ids` の件数より少なくなるので、Nyx は件数の一致で成否を判定せず `order_id` の集合で照合すること |
