...
| 絞り込みパラメータの不正値 | `active_orders` / `trade_history` の `count` / `from_id` / `end_id` / `since` / `end` は、**空文字を「未指定」や `0` として扱わず**、パラメータごとの error code で断る（400）。`count` `40006` / `end` `40007` / `end_id` `40008` / `from_id` `40009` / `since` `40022`。数値として読めない値（`?count=abc`）も同じコード。指定が無いときの挙動は従来どおり | **実 API を実測**（2026-09-16、`btc_jpy`、認証済みの口座）: `active_orders?count=` → `40006`、`active_orders?end=` → `40007`、`trade_history?since=` → `40022`。コードの定義は errors.md（`0badd680`）の `"Invalid count."` / `"Invalid end param."` / `"Invalid end_id."` / `"Invalid from_id."` / `"Invalid trading start time."` | 差異なし（この 3 つは実測で一致）。**`end_id` / `from_id` と、実測していないエンドポイント側の組み合わせは errors.md のコード名からの推測**。複数のパラメータが同時に不正なときに実 API がどれを返すかも未実測 | 一部はい（上記） | v0.1.0 からの**改訂**。旧版は `z.coerce.number()` が `""` を `0` にするため、**`?end=` が `success: 1` のまま常に空配列**を返していた（`0` 以下だけを残す絞り込みになる）。`?count=` は汎用の `20003` だった。Nyx が絞り込みを使うなら、値が空になり得る変数をクエリへ入れないこと。複数不正時のコード選択に依存しないこと |
