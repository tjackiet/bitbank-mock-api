...
- **本物との差異**: **超過したとき実 API がこの番号を返すことは実測していない**（取消の実測には実弾の注文が要る）。errors.md の意味から選んだ**推測**である。**断ったときに 1 件も取り消さないことも未実測**で、`cancel_orders` のバッチ全体を fail-closed にする既存の判断（上の「取消済み・約定済みの取消」節）に揃えた。**31 件のうち 1 件が不正値だったときどちらのコードが勝つかも未実測**——モックは件数の検査をスキーマ検証の後に置いたので `20003` が勝つが、これは既存の（実測済みの）優先順を動かさない位置に足した結果であって、実 API の優先順の再現ではない
...
残高不足 `60001`。欠落: amount `30001`、price `30012`、side `30013`、type `30015`、order_id `30006`、order_ids `30007`、**pair `30009`**。絞り込みパラメータの不正値は専用コード（`40006` / `40007` / `40008` / `40009` / `40022`。上の「絞り込みパラメータの不正値」節）。**不正なペアは `40017`**。それ以外の不正値は `20003`（公式の ACCESS-KEY 欠落コードをパラメータエラーに流用）。**非正の `amount` / `price` は「欠落」ではなく「不正値」として扱う**ので `20003` で、欠落の `30001` / `30012` とは別のコードになる（`0` を「未指定」に丸めない）。パラメータの値ではなく**要求そのものを断る**コードが 3 つあり、こちらは別の節が根拠と留保を持つ: `cancel_orders` の件数超過は `40015`（上の「一括取消の件数上限」節）、active な注文が 30 本ある口座からの新規発注は `60011`（上の「同時未約定注文の上限」節）、発注停止のペアへの新規発注は `70017`（下の「ペア」節）
...
- **採ったコードは `70017`「Orders on pair have been suspended.」（`errors.md:225`）で、意味は一致する。ただし推測である。** **停止ペアへ実際に発注したとき実 API が返すコードは実測していない**（発注は実弾になるため測れない）。桁（`60004` / `20003`）と同時に不正なときどちらが勝つかも未実測で、モックは実在性の検査の直後・桁の検査より前に置いている。実 API の優先順の再現ではない
