...
| control の時計 | `POST /_control/tick` は状態の `lastTickAt` を `max(現在時刻, 前回 + 60 秒, 足の timestamp)` へ進める。1 回の tick で必ず 60 秒以上進み、**巻き戻らない**。`timestamp` の上限は `8.64e15 − 9 時間`（`Date` の表現範囲から JST オフセットを引いた値。同じ表の「control の fill / tick 検証」行）だけなので、`1e15` のような値を渡すと `lastTickAt` が西暦 33658 になり、以後どんな `timestamp` を渡しても戻らない。**戻す手段は `POST /_control/reset`（注文・約定・残高を全部捨てる）だけ。** ただし `BITBANK_MOCK_FILL_MODE=market` のときに限り、互換ルートが呼ぶ `SessionStore.tick()` が `lastTickAt` を現在時刻で上書きするので、未来へ進めた時計が巻き戻る。なお `lastTickAt` が `8.64e15 − 9 時間` を超えると、market モードの足取得が日付を JST へ寄せる際に `Invalid Date` になり、URL の日付が `NaNNaNNaN` になって取得が毎回失敗する（例外にはならない）。足の `timestamp` はこの上限で弾くが、その上限ちょうどの足を受けた後に tick を重ねると `lastTickAt` だけは 60 秒ずつ超えていくので、時計そのものに上限を入れない限りこの経路は閉じない | 本モック固有 | 本物の取引所には対応する概念がない | はい | **未確定。** 「現在時刻からどれだけ先の足まで control に許すか」は決めていない。上限（例: 現在時刻 + 24 時間）を入れるか、時計を戻す control 操作を足すかは根拠が無いので保留し、現状は「`8.64e15 − 9 時間` までの足なら通し、reset でだけ戻せる」を選んでいる。Nyx 側は `lastTickAt` を実時間と見なさない |
| control の fill / tick 検証 | 存在しない注文 404、終端 409。`POST /_control/tick` の `pair` は互換ルートと同じ検証（`pairAssets`）を通らなければ 400 `INVALID_PAIR`。`amount` が非正・残量超過・桁溢れは 400 `INVALID_AMOUNT`。`price` が非正・非有限は 400 `INVALID_PRICE`。足は `0 < low <= open <= high` かつ `low <= close <= high` の有限値で、`timestamp` は `Date` の表現範囲から下流の加算分を引いた範囲（`-8.64e15 <= t <= 8.64e15 − 9 時間`。上側だけ JST オフセット分の余裕を取るので非対称）に収まること。`POST /_control/reset` の `balances` のキーは互換ルートと同じ文字種（`[a-z0-9]+`、`pairAssets` のセグメント）に限り、外れるものは 400 `INVALID_BALANCES`。拒否時は状態を変えない（`fill` / `tick` / `reset` の全拒否経路で確認済み） | 本モック固有（不変量 1 の防御） | 本物には無い | はい | 実験用の部分約定は control からのみ起こす。DCL の通常経路では使わない |
